#!/usr/bin/env python3
"""كتابة الدفعة الثانية (43 اسماً، الطبقة الثامنة) من تذكرة الحفاظ للذهبي عبر upsert_story_draft بمفتاح anon."""
...
SCRATCH = "/tmp/claude-1000/-home-user-projects-siraj/5b3301c6-b957-4875-a5a0-f28d2e514459/scratchpad/batch2_tadhkira"
AUTHOR = "الذهبي"
SOURCE_BOOK = "تذكرة الحفاظ"
SRC_URL = "https://ar.wikisource.org/wiki/" + urllib.parse.quote("تذكرة الحفاظ/الطبقة الثامنة".replace(" ", "_"))
...
def main():
    files = sorted(glob.glob(f"{SCRATCH}/*.txt"))
    next_idx = 326
    results = []
    for fpath in files:
        fname = fpath.split("/")[-1]
        person_name = fname.replace(".txt", "").split("_", 1)[1]
