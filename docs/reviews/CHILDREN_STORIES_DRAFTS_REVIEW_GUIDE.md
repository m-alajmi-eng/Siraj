...
## 5) صياغة مسودة أولى بمساعدة أداة الذكاء الاصطناعي (draft_story.py)

سكربت مستقل في `tools/children_stories_draft/draft_story.py` يساعد على صياغة
النص المبسَّط لأي من التسع قصص انطلاقاً من نص مصدري حقيقي من مكتبتنا الموثوقة
(ابن كثير/القرآن/الحديث) يقصّه المالك يدوياً - **لا حصاد آلي ولا مصدر خارجي
بأي حال**.

**تحذير صريح (يتكرر عمداً في كل مكان يخص هذه الأداة):** كل ما تنتجه هذه الأداة
مسودة ذكاء اصطناعي دائماً - مهما بدت الصياغة دقيقة أو "مجرد تلخيص حرفي"، فهي
لا تُنشَر أبداً بلا قراءتك الفعلية الكاملة لكل كلمة فيها بنفسك. لا مسار آلي
في السكربت يتجاوز هذا - انظر التصميم بخطوتين أدناه.

### كيف يشغّلها المالك

```bash
export ANTHROPIC_API_KEY=...        # مطلوب لخطوة generate
export SUPABASE_URL=...             # مطلوب لخطوة commit فقط
export SUPABASE_ANON_KEY=...        # مطلوب لخطوة commit فقط (مفتاح anon العام، لا مفتاح خدمة)

# 1) التوليد - يطبع المسودة على الشاشة ويحفظها في ملف محلي، لا كتابة على القاعدة إطلاقاً
python3 tools/children_stories_draft/draft_story.py generate \
  --source-file /path/to/original_source.txt \
  --title "أصحاب الكهف"
#   → يحفظ في tools/children_stories_draft/drafts/<...>.json (غير مرفوع لـgit)

# 2) الكتابة - بعد المراجعة (والتعديل اليدوي إن أردت) على الملف السابق
python3 tools/children_stories_draft/draft_story.py commit \
  --draft-file tools/children_stories_draft/drafts/<...>.json
```

`generate` و`commit` خطوتان منفصلتان عمداً - لا يوجد أمر واحد يولّد وينشر
معاً. هذا يضمن وجود توقف حقيقي بينهما لمراجعتك.

### كيف تراجع المسودة قبل القبول

1. بعد `generate`، اقرأ النص المطبوع على الشاشة **كاملاً** (لا تكتفِ بأول
   سطرين) - تحقّق أنه لا يضيف حدثاً أو حواراً أو شخصية أو تفصيلاً غير موجود
   صراحة في نصك المصدري، ولا يتضمن أي اجتهاد فقهي أو عقدي.
2. يمكنك فتح الملف المحفوظ `tools/children_stories_draft/drafts/<...>.json`
   وتعديل حقل `simplified_text` (أو `reading_time_minutes`) يدوياً بأي قدر
   تريده قبل `commit` - السكربت يقرأ الملف كما هو وقت `commit`، فأي تعديل
   يدوي عليه يُحترَم بالكامل.
3. عند تشغيل `commit`، يطبع السكربت محتوى الملف كاملاً مرة أخيرة، ويطلب منك
   كتابة كلمة **"نعم"** حرفياً للتأكيد - أي إدخال آخر يُلغي العملية بلا كتابة
   شيء. هذه هي نقطة الموافقة اليدوية الفعلية.
4. `commit` يكتب الصفّ بـ`reviewed = false` دائماً (نفس مسار `upsert_draft_story`
   في القسم 2 أعلاه، بما في ذلك تنبيه إدراج صفّ جديد دوماً لا تحديث لصفّ
   قائم). **النشر الفعلي للعموم يبقى دائماً خطوة يدوية منفصلة تماماً** - القسم
   (4) أعلاه (`reviewed = true`) - بعد قراءتك الكاملة للنص من داخل القاعدة
   نفسها، لا فقط من مخرجات السكربت.
